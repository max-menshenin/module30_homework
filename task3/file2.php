...
if (!empty($_POST['comment'])) {
	...
	if (empty($errors)) {
		...
		$comment = 'Автор: ' . $row['user_login'] . ',' . date('d.m.y H:i') . ': ' . $comment;
...
<body class="file">
	...
	<div class="container pt-4">
		...
		<h1 class="mb-4 fileh"><a href="./gallery.php">Галерея изображений</a></h1>
			...
			<div class="col-12 col-sm-8 offset-sm-2">
				...
				<?php if (!empty($comments)) : ?>
					<?php foreach ($comments as $key => $comment) : ?>
						<form>
							<p type="hidden" class="<?php echo (($key % 2) > 0) ? 'bg-light' : ''; ?>"><?php echo $comment; ?></p>
					...
					<?php endforeach; ?>
				<?php else : ?>
					<p class="text-muted">Пока ни одного комментария, будьте первым!</p>
				<?php endif; ?>
